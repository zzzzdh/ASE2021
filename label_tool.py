...
class right_box(BoxLayout):
  ...
  def get_video_list(self):
    ...
    video_list = [x.split('.')[0] for x in os.listdir(os.path.join(data_dir, 'Diff')) if x.split('.')[0] not in exist_list]
    video_list.sort()
    return video_list

  def get_image_list(self, video_name):
    image_list = []
    with open(os.path.join(data_dir, 'Diff', video_name + '.txt')) as diff_file:
